...
public class ImpostazioniFragment extends AbFrg implements DatePickerDialog.OnDateSetListener {
    ...
    private String codice_utente = "1234";
    private String nome;
    ...
    protected View viewOfFragment(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        layout = (LinearLayout) root.findViewById(R.id.layout_impostazioni);

        save.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (cf.getText().toString().isEmpty()||name.getText().toString().isEmpty() ||surname.getText().toString().isEmpty()
                    ...
                    codiceFiscale = cf.getText().toString();
                    nome = name.getText().toString();
                    cognome = surname.getText().toString();
                    data_nascita = dateText.getText().toString();
                    pass = password.getText().toString();

                    String currentString = data_nascita;
                    ...
                    if(radioSelected.equalsIgnoreCase("M")){
                        sesso = "u";
                    }else{
                        sesso = "d";
                    }
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try{
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                emailCaregiver = object.getString("caregiver_email");
                                codiceFiscale = object.getString("cf");
                                sesso = object.getString("sesso");
                                data_nascita = object.getString("data_nascita");
                                data_nascita = changeDataDB(data_nascita);

                                String immagineProfilo = object.getString("immagine_profilo");
                                immagine = immagineProfilo.replace("data:image/jpeg;base64,","");
                                viewImage.setImageBitmap(com.example.logo.util.Bitmap.getBitmapFromString(immagine));
    ...
    public void update_paziente(){

        String urlPaziente = "http://logopediapp.altervista.org/database/crud_paziente/update.php";

        //String newimage = "data:image/jpeg;base64," + immagine;

        JSONObject json = new JSONObject();
        try {
            json.put("codice", codice_utente);
            json.put("cf", codiceFiscale);
            json.put("nome", nome);
    ...
    public String changeDataDB(String data) {
        String[] separated = data.split("-");
        String day = separated[2];
        String month = separated[1];
        String year = separated[0];

        String finalData = day + "/" + month + "/" + year;
    ...
    public Bitmap getResizedBitmap(android.graphics.Bitmap image, int maxSize) {
    ...
    private void snackbarError(){
        Snackbar.make(getActivity().findViewById(android.R.id.content), "Paziente aggiornato correttamente!", Snackbar.LENGTH_LONG).show();
    }
}
